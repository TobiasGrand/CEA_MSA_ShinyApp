...
# setwd("C:/Users/togn/OneDrive - H. Lundbeck A S/University of Sheffield/Part III Cost-effectiveness modelling MSA/Markov Model MSA/Markov Model MSA Final/App") # Set working directory
...
ui <- navbarPage( # Opens the page navbar 
  ...
  nav_spacer(), # Adds space between the sidebar and the main panel
    ...
    nav_panel ("Sensitivity Analysis",
               ...
               tags$p("All parameters from the PSA are available for download as a csv file. When reading the file note that comma is (.) and separator is (,) which is important."),
               downloadButton("DownloadAllParameters", "Download All PSA Parameters")
...
server <- function(input, output, session) {
 ...
 observeEvent(input$run_model, ignoreNULL = F, {
   ...
   df_all_parameters <- results$all_parameters
   
   # Download as csv for checking results and double coding in Excel. PSA results are also available from App/Data folder. 
   
   output$DownloadAllParameters <- downloadHandler(
     filename = function() {
       paste("PSA_all_parameters_", format(Sys.time(), "%Y%m%d"), ".csv", sep="")
     },
     content = function(file) {
       # Check if the data exists
       if (exists("df_all_parameters")) {
         write.csv(df_all_parameters, file, row.names = FALSE)
       } else {
         # Create empty file with message if no data
         write.csv(data.frame(message="No PSA has been run yet"), file, row.names = FALSE)
       }
     }
   )
